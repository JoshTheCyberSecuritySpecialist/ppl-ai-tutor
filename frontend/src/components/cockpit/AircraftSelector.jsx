...
const cockpitAircraft = [
  { id: "c172", name: "Cessna 172", role: "Baseline trainer", notes: "Classic analog six-pack trainer." },
  { id: "archer", name: "Piper Archer", role: "Cross-country trainer", notes: "Low-wing trainer with IFR focus." },
  { id: "da40", name: "Diamond DA40", role: "TAA trainer", notes: "Modern glass cockpit for advanced training." },
];
...
export function AircraftSelector({ selectedId, onSelect }) {
  return (
    <div className="flex h-full flex-col rounded-2xl border border-slate-800 bg-slate-900/80 p-4">
      <div className="mb-3">
        <h3 className="text-sm font-semibold text-slate-100 md:text-base">Aircraft Selector</h3>
        <p className="text-xs text-slate-400">
          Pick the training aircraft whose cockpit layout and instruments you want to explore.
        </p>
      </div>
      <div className="flex-1 space-y-2 overflow-y-auto pr-1 text-xs md:text-sm">
        {cockpitAircraft.map((aircraft) => {
